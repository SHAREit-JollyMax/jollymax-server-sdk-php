...
namespace jollymax\sdk\client;

use http\Exception\RuntimeException;
use jollymax\sdk\constants\Env;
use jollymax\sdk\domain\GatewayReq;
use GuzzleHttp\Client;
use jollymax\sdk\domain\GatewayResult;
use jollymax\sdk\utils\RSAUtils;

class JollymaxClient
{
    ...
    public static function send($apiName, $data) {
        ...
        if(GatewayResult::success($respJson)
            ...
            self::$merchantConfig->jollymaxPublicKey)) {
            return $respBody;
        }
    ...
    public static function verify($body, $sign) {
        if(empty(self::$merchantConfig) || empty(self::$merchantConfig->jollymaxPublicKey)) {
            throw new \RuntimeException("请配置jollymax公钥");
        }

        return RSAUtils::verify($body, $sign, self::$merchantConfig->jollymaxPublicKey);
    }
